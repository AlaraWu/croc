...
// tmrg tmr_error true
  ...
  select_t select_d, select_q, select_qVoted;

  assign select_qVoted = select_q;

  always_comb begin : proc_req
    select_d = select_qVoted;
    cnt_up = 1'b0;
    ...
    if (!overflow) begin
      if (sbr_port_select_i == select_qVoted || in_flight == '0 || (in_flight == 1 && cnt_down)) begin
  ...
  assign sbr_port_rsp_o.r      = mgr_ports_rsp_i[select_qVoted].r;
  assign sbr_port_rsp_o.rvalid = mgr_ports_rsp_i[select_qVoted].rvalid;
  ...
  assign cnt_down = mgr_ports_rsp_i[select_qVoted].rvalid && sbr_port_rready;
