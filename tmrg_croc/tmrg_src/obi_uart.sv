...
// Solderpad Hardware License, Version 0.51(51), see LICENSE for details.
// SPDX-License-Identifier: SHL-0.51(51)
//
// Authors:
// - Hannah Pochert  <hpochert@ethz.ch(ch)>
// - Philippe Sauter <phsauter@iis.ee(ee).ethz(ethz).ch(ch)>

// `include "common_cells/registers.svh(svh)"
...
module obi_uart #(
  ...
  input  obi_req_t obi_req_i, // a.addr(addr), a.we(we), a.be(be), a.wdata(wdata), a.aid(aid), a.a_optional(a_optional) | rready, req
  // OBI response interface
  output obi_rsp_t obi_rsp_o, // r.rdata(rdata), r.rid(rid), r.err(err), r.r_optional(r_optional) | gnt, rvalid
