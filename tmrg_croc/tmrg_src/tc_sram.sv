...
// tmrg tmr_error true
  ...
  addr_t [NumPorts-1:0] r_addr_q, r_addr_qVoted;
  ...
  data_t [NumPorts-1:0][Latency-1:0] rdata_q,  rdata_d, rdata_qVoted;
  if (Latency == 32'd0) begin : gen_no_read_lat
    for (genvar i = 0; i < NumPorts; i++) begin : gen_port
      assign rdata_o[i] = (req_i[i] && !we_i[i]) ? sram[addr_i[i]] : sram[r_addr_qVoted[i]];
    end
  end else begin : gen_read_lat

    always_comb begin
      for (int unsigned i = 0; i < NumPorts; i++) begin
        rdata_o[i] = rdata_qVoted[i][0];
        for (int unsigned j = 0; j < (Latency-1); j++) begin
          rdata_d[i][j] = rdata_qVoted[i][j+1];
        end
        rdata_d[i][Latency-1] = (req_i[i] && !we_i[i]) ? sram[addr_i[i]] : sram[r_addr_qVoted[i]];
      end
    end
  end
