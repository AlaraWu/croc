...
// tmrg tmr_error true
   ...
   logic [31:0]        s_count, s_count_reg, s_count_regVoted;
   assign s_count_regVoted = s_count_reg;

   logic target_reached_q, target_reached_qVoted;
   assign target_reached_qVoted = target_reached_q;
   assign target_reached_o = target_reached_qVoted;

   // COUNTER
   always_comb
   begin
      s_count = s_count_regVoted;
   ...
   begin
      if (rst_ni == 0)
         target_reached_q <= 1'b0;
      else
         if ( s_count == compare_value_i )
            target_reached_q <= 1'b1;
         else
            target_reached_q <= 1'b0;
   end
   
   assign counter_value_o = s_count_regVoted;
