...
module fault_monitor_reg_top #(
    ...
    always_comb begin
        ...
        is_invalid_rw = '0;
        decoded_reg_strb.obi_fault = cpuif_req_masked & (cpuif_addr == 5'h0);
        decoded_reg_strb.core_fault = cpuif_req_masked & (cpuif_addr == 5'h4);
    ...
    // Field: fault_monitor.obi_fault.fault_count
    always_comb begin
        automatic logic [31:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.obi_fault.fault_count.value;
        load_next_c = '0;
        if(decoded_reg_strb.obi_fault && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.obi_fault.fault_count.value & ~decoded_wr_biten[31:0]) | (decoded_wr_data[31:0] & decoded_wr_biten[31:0]);
            load_next_c = '1;
        end
        if(hwif_in.obi_fault.fault_count.incr) begin // increment
            field_combo.obi_fault.fault_count.overflow = (((33)'(next_c) + 32'h1) > 32'hffffffff);
            next_c = next_c + 32'h1;
            load_next_c = '1;
        end else begin
            field_combo.obi_fault.fault_count.overflow = '0;
        end
        field_combo.obi_fault.fault_count.incrthreshold = (field_storage.obi_fault.fault_count.value >= 32'hffffffff);
        field_combo.obi_fault.fault_count.next = next_c;
        field_combo.obi_fault.fault_count.load_next = load_next_c;
    end
    always_ff @(posedge clk or negedge arst_n) begin
        if(~arst_n) begin
            field_storage.obi_fault.fault_count.value <= 32'h0;
        end else begin
            if(field_combo.obi_fault.fault_count.load_next) begin
                field_storage.obi_fault.fault_count.value <= field_combo.obi_fault.fault_count.next;
            end
        end
    end
    // Field: fault_monitor.core_fault.fault_count
    always_comb begin
        automatic logic [31:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.core_fault.fault_count.value;
        load_next_c = '0;
        if(decoded_reg_strb.core_fault && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.core_fault.fault_count.value & ~decoded_wr_biten[31:0]) | (decoded_wr_data[31:0] & decoded_wr_biten[31:0]);
            load_next_c = '1;
        end
        if(hwif_in.core_fault.fault_count.incr) begin // increment
            field_combo.core_fault.fault_count.overflow = (((33)'(next_c) + 32'h1) > 32'hffffffff);
            next_c = next_c + 32'h1;
            load_next_c = '1;
        end else begin
            field_combo.core_fault.fault_count.overflow = '0;
        end
        field_combo.core_fault.fault_count.incrthreshold = (field_storage.core_fault.fault_count.value >= 32'hffffffff);
        field_combo.core_fault.fault_count.next = next_c;
        field_combo.core_fault.fault_count.load_next = load_next_c;
    end
    always_ff @(posedge clk or negedge arst_n) begin
        if(~arst_n) begin
            field_storage.core_fault.fault_count.value <= 32'h0;
        end else begin
            if(field_combo.core_fault.fault_count.load_next) begin
                field_storage.core_fault.fault_count.value <= field_combo.core_fault.fault_count.next;
            end
        end
    end
    ...
    for(genvar i0=0; i0<2; i0++) begin
        ...
        always_comb begin
            automatic logic [31:0] next_c;
            automatic logic load_next_c;
            next_c = field_storage.sram_fault[i0].fault_count.value;
            load_next_c = '0;
            if(decoded_reg_strb.sram_fault[i0] && decoded_req_is_wr) begin // SW write
                next_c = (field_storage.sram_fault[i0].fault_count.value & ~decoded_wr_biten[31:0]) | (decoded_wr_data[31:0] & decoded_wr_biten[31:0]);
                load_next_c = '1;
            end
            if(hwif_in.sram_fault[i0].fault_count.incr) begin // increment
                field_combo.sram_fault[i0].fault_count.overflow = (((33)'(next_c) + 32'h1) > 32'hffffffff);
                next_c = next_c + 32'h1;
                load_next_c = '1;
            end else begin
                field_combo.sram_fault[i0].fault_count.overflow = '0;
            end
            field_combo.sram_fault[i0].fault_count.incrthreshold = (field_storage.sram_fault[i0].fault_count.value >= 32'hffffffff);
            field_combo.sram_fault[i0].fault_count.next = next_c;
            field_combo.sram_fault[i0].fault_count.load_next = load_next_c;
        end
        always_ff @(posedge clk or negedge arst_n) begin
            if(~arst_n) begin
                field_storage.sram_fault[i0].fault_count.value <= 32'h0;
            end else begin
                if(field_combo.sram_fault[i0].fault_count.load_next) begin
                    field_storage.sram_fault[i0].fault_count.value <= field_combo.sram_fault[i0].fault_count.next;
                end
            end
        end
    end
